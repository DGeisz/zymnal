...
const InPlaceSymbolsId = "ip2";

enum InPlaceMessageType {
  AddSlashMap = "asm",
  AddDirectMap = "adm",
}
...
export const InPlaceMessage = {
  addSlashMap(content: InPlaceSymbolMap): HermesMessage<InPlaceSymbolMap> {
    return {
      zentinelId: InPlaceSymbolsId,
      message: InPlaceMessageType.AddSlashMap,
      content,
    };
  },
  addDirectMap(content: InPlaceSymbolMap): HermesMessage<InPlaceSymbolMap> {
    return {
      zentinelId: InPlaceSymbolsId,
      message: InPlaceMessageType.AddDirectMap,
      content,
    };
  },
};

class InPlaceSymbols extends Zentinel {
  zyId: string = InPlaceSymbolsId;
      ...
      TransformerMessage.registerTransformerFactory({
        ...
        factory: async (treeRoot, cursor) => {
          ...
          await cursorForEach(treeRoot, cursor, async (zym) => {
            const transResult = await zym.cmd<ContextualTransformers>(
              InPlaceCommand.getInPlaceSymbolMaps
            );

            if (transResult.ok) {
              const { slash, direct } = transResult.val;
          ...
          return [
            async (root: Zymbol, cursor: Cursor, keyPress: ZymKeyPress) => {
              ...
              console.log([...this.globalSlashMap, ...contextSlash]);
  ...
  handleMessage = async (msg: ZentinelMessage): Promise<ZyResult<any>> => {
    switch (msg.message) {
      case InPlaceMessageType.AddSlashMap: {
        const { id } = msg.content! as InPlaceSymbolMap;

        if (!this.globalSlashMap.some((s) => _.isEqual(s.id, id))) {
          this.globalSlashMap.push(msg.content);
        }

        return ok(NONE);
      }
      case InPlaceMessageType.AddDirectMap: {
        const { id } = msg.content! as InPlaceSymbolMap;

        if (!this.globalDirectMap.some((s) => _.isEqual(s.id, id))) {
          this.globalDirectMap.push(msg.content);
        }
...
/* Commands that get different transformations */
interface InPlaceCommandGroup extends ZyCommandGroupType {
...
const pathFactory = groupPathFactory(InPlaceSymbolsId);

export const InPlaceCommand: ZyCommandGroup<InPlaceCommandGroup> = {
  getInPlaceSymbolMaps: justPath(pathFactory("gipt")),
};
